...
<p>손절 후에는 반드시 <strong>매매 일지를 작성</strong>해야 합니다. 왜 손실이 발생했인지, 어떤 실수를 했인지 기록하고 분석해야 같은 실수를 반복하지 않습니다. 손절은 실패가 아니라 <strong>학습의 기회</strong>입니다.</p>
...
<div class="internal-links" style="margin-top: 2rem; padding: 1.5rem; background: rgba(255,255,255,0.05); border-radius: 12px; border-left: 4px solid var(--accent-color);"><h4 style="margin-top: 0; margin-bottom: 1rem;">💡 함께 읽어보면 좋은 글</h4><ul style="list-style: none; padding: 0; margin: 0;"><li style="margin-bottom: 0.5rem;"><a href="blog.html?id=18" style="color: var(--accent-color); text-decoration: none; font-weight: 600;">🔗 [따끔한 질책] '물타기'면 다 되는 줄 아십니까? 그건 매우 위험한 행위입니다.</a></li><li style="margin-bottom: 0.5rem;"><a href="blog.html?id=21" style="color: var(--accent-color); text-decoration: none; font-weight: 600;">🔗 [마인드셋] 매매 일지 안 쓰는 투자자는 같은 실수를 반복하며 돈을 잃습니다.</a></li></ul></div>`
};
...
<ul>
  ...
  <li><strong>배당 증가율:</strong> 최근 5년간 배당금이 얼마나 증가했인지 확인하세요. 꾸준히 증가하는 기업이 좋습니다.</li>
...
if (post31Match) {
    const oldPost31 = post31Match[0].slice(0, -3);
    updatedPost = JSON.stringify(jan23NewPost, null, 8).replace(/\n/g, '\n        ');
    content = content.replace(oldPost31, updatedPost);
}

if (post32Match) {
    const oldPost32 = post32Match[0].slice(0, -3);
    updatedPost = JSON.stringify(jan24NewPost, null, 8).replace(/\n/g, '\n        ');
    content = content.replace(oldPost32, updatedPost);
}

if (post33Match) {
    const oldPost33 = post33Match[0].slice(0, -3);
    updatedPost = JSON.stringify(jan25NewPost, null, 8).replace(/\n/g, '\n        ');
    content = content.replace(oldPost33, updatedPost);
}
