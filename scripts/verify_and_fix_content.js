...
검증 항목:
1. **날짜 정확성**: 콘텐츠가 오늘(${today}) 기준인가?
2. **데이터 정확성**: 실제 시장 데이터와 일치하는가?
3. **금지 표현**: "저", "제가", "나", "주식 선배" 등 1인칭 표현이 있는가?
4. **인코딩 문제**: 깨진 문자나 이상한 기호가 있는가?
5. **과거 시제**: "어제", "지난주", "했습니다" 등 과거 표현이 있는가?
6. **문법 및 자연스러움**: 문장이 자연스럽고 문법적으로 올바른가?

JSON 형식으로만 응답:
{
  "passed": true/false,
  "issues": [
    {"type": "날짜", "severity": "high/medium/low", "description": "구체적 문제", "location": "시장브리핑/블로그"},
    ...
  ],
  "score": 0-100,
...
**수정 지침**:
1. 1인칭 표현 제거 ("저", "제가" → 제거 또는 "투자자들", "시장 참여자들"로 변경)
2. 과거 시제 → 현재 시제 변환
3. 깨진 문자 복구
4. 부정확한 데이터 수정
5. 문법 및 자연스러움 개선
